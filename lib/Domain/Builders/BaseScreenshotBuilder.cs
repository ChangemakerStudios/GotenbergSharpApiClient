    ...
    where TBuilder : BaseScreenshotBuilder<TRequest, TBuilder>
{
    ...
    public TBuilder SetScreenshotBehaviors(Action<ScreenshotBehaviorBuilder> action)
    {
        if (action == null) throw new ArgumentNullException(nameof(action));
        action(new ScreenshotBehaviorBuilder(this.Request.ScreenshotBehaviors));
        return (TBuilder)this;
    }

    public TBuilder SetScreenshotBehaviors(ScreenshotBehaviors behaviors)
    {
        this.Request.ScreenshotBehaviors =
            behaviors ?? throw new ArgumentNullException(nameof(behaviors));
        return (TBuilder)this;
    }
}
